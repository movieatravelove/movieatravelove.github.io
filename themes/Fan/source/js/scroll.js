(function () {
    ...
    function findHeadPosition(el) {
        ...
        list.each(function () {
            if ($(this).offset().top <= menuheight)="" {="" currentid="$(this).attr('id');" }="" });="" $('.sidebar-toc-content="" .toc-link').removeclass('active');="" if="" (currentid="==" '')="" return;="" let="" $this="$('.sidebar-toc-content" .toc-link[href="#' + currentId + '" ]');="" $this.addclass('active');="" parents="$this.parents('.toc-child');" (parents.length=""> 0) {
            let child = null;
